...
function fakeGmail(
  ...
  captured: { params?: gmail_v1.Params$Resource$Users$Settings$Updatepop },
): gmail_v1.Gmail {
  return {
    users: {
      settings: {
        updatePop: async (params: gmail_v1.Params$Resource$Users$Settings$Updatepop) => {
...
describe('update_pop', () => {
  it('passes the account parameters and projects the settings', async () => {
    const captured: { params?: gmail_v1.Params$Resource$Users$Settings$Updatepop } = {};
  ...
  it('rejects unspecified and unknown accessWindow', () => {
    expect(() => schema.input.parse({ accessWindow: 'accessWindowUnspecified' })).toThrow();
    expect(() => schema.input.parse({ accessWindow: 'futureValue' })).toThrow();
  });

  it('rejects unspecified and unknown disposition', () => {
    expect(() => schema.input.parse({ disposition: 'dispositionUnspecified' })).toThrow();
    expect(() => schema.input.parse({ disposition: 'futureValue' })).toThrow();
  });
});
