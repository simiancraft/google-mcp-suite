...
function fakeGmail(
  data: object,
  captured: { params?: gmail_v1.Params$Resource$Users$Settings$Cse$Identities$Patch },
): gmail_v1.Gmail {
  return {
    users: {
      settings: {
        cse: {
          identities: {
            patch: async (params: gmail_v1.Params$Resource$Users$Settings$Cse$Identities$Patch) => {
...
describe('patch_cse_identity', () => {
  it('passes exact account parameters and projects documented output', async () => {
    const captured: { params?: gmail_v1.Params$Resource$Users$Settings$Cse$Identities$Patch } = {};
    ...
    expect(captured.params).toEqual({
      userId: 'me',
      requestBody: { primaryKeyPairId: 'key-1' },
      emailAddress: 'me@example.com',
    });
  ...
  it('accepts separate signing and encryption keys and rejects conflicting configurations', async () => {
    for (const signAndEncryptKeyPairs of [
      ...
      expect(captured.params?.requestBody).toEqual({ signAndEncryptKeyPairs });
      expect(() => schema.output.parse(result)).not.toThrow();
    }
    expect(() =>
      schema.input.parse({
